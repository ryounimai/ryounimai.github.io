/**
 * components/watch.js — ŘΨØŬ v1.0.2
 * Video.js 8 player + sidebar episode list + progress + autoplay
 */

// ── Format waktu: selalu 2 digit (00:30 bukan 0:30) ──────────────────────────
...
<div class="watch-layout">
  <div class="watch-col-main">
    <div class="watch-player-box" id="watch-player-box">
      <video id="ryou-player" class="video-js vjs-ryou vjs-big-play-centered" playsinline webkit-playsinline crossorigin="anonymous" controlsList="nodownload nofullscreen noremoteplayback" oncontextmenu="return false" disablePictureInPicture poster=""></video>
    </div>
...
${next ? `<button class="btn btn-ghost btn-sm" id="btn-next-ep">EP ${next.ep} ›</button>` : `<button class="btn btn-ghost btn-sm" disabled>${I18n.t('next_ep').replace(' ›','')}</button>`}`;
  ...
  function _bindEpList() {
    // Use event delegation on ws-list to avoid rebind issues
    const list = document.getElementById('ws-list');
    if (!list) return;
    // Remove old listener by cloning (safest way)
  ...
  function _recreateVideoEl() {
    // dispose() removes the <video> from DOM entirely — recreate it
    ...
    v.setAttribute('poster', '');  // block Android WebView auto-poster CORS
    box.appendChild(v);
  }
  ...
  function _initPlayer(ep, anime) {
    ...
    // Block right-click & long-press download on player
    // Fix HUD: Video.js binds to document — override so controls only
    // appear when interaction is INSIDE the player element
    _player.ready(() => {
      const el = _player.el();

      // ── DRM: block context menu & iOS callout ──
    ...
    if (settings.subtitles !== false) {
      (ep.subtitles || []).forEach(sub => {
        _player.addRemoteTextTrack({ kind:'subtitles', label:sub.label||sub.lang, srclang:sub.lang||'id', src:sub.src, default:sub.default||false }, false);
      });
    }
    ...
    // Progress save — debounced every 5s
    let _lastSave = 0;
